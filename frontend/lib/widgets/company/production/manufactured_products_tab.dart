...
class _ManufacturedProductsTabState extends ConsumerState<ManufacturedProductsTab> {
  ...
  await Future.wait([_loadProducts(), _loadAllProducts(), _loadAccounts()]); // 👈 ДОБАВИТЬ _loadAccounts
...
Future<void> _loadAccounts() async {
  try {
    ...
    // Можно сохранить в state или просто обновить
  ...
  Future<void> _loadProducts() async {
    try {
      ...
      print('✅ Products loaded: ${res.data}');
      if (mounted) {
              ...
              onPressed: saleItems.isEmpty ? null : () async {
                ...
                try {
                  for (var item in saleItems) {
                    ...
                    print('📦 Selling product: ${item['name']}');
                    print('   product_id: ${item['id']}');
                    print('   quantity: $quantity');
                    print('   amount: $amount');
                    print('   account_id: $selectedAccountId');
                    print('   date: ${date.toIso8601String()}');
                    print('   counterparty: ${counterpartyController.text.isEmpty ? null : counterpartyController.text}');
                    
                    final response = await _api.post('/production/sell', 
                    ...
                    print('✅ Sell successful: $response');
                  }
                  if (mounted) {
