...
class CashAccountMovementReport extends ConsumerStatefulWidget {
  final int companyId;
  const CashAccountMovementReport({super.key, required this.companyId});

  @override
  ConsumerState<CashAccountMovementReport> createState() => _CashAccountMovementReportState();
}

class _CashAccountMovementReportState extends ConsumerState<CashAccountMovementReport> {
  ...
  Future<void> _getCashAccountId() async {
    try {
      final res = await _api.get('/accounts', queryParameters: {'company_id': widget.companyId});
      final accounts = res.data as List;
      final cashAcc = accounts.firstWhere((a) => a['type'] == 'cash', orElse: () => null);
      if (cashAcc != null) setState(() => _cashAccountId = cashAcc['id']);
    ...
    if (bytes != null) {
      if (kIsWeb) {
        ...
        final anchor = html.AnchorElement(href: url)
          ..setAttribute('download', 'cash_movement.xlsx')
          ..click();
        html.Url.revokeObjectUrl(url);
      } else {
        final dir = await getTemporaryDirectory();
        final path = '${dir.path}/cash_movement_${DateTime.now().millisecondsSinceEpoch}.xlsx';
