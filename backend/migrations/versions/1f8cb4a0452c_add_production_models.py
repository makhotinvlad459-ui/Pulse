...
def upgrade() -> None:
    ...
    op.execute("CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_manufactured_products_company ON manufactured_products(company_id);")
    op.execute("CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_manufactured_products_deleted ON manufactured_products(is_deleted);")
    op.execute("CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_production_journal_date ON production_journal_entries(production_date);")
    op.execute("CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_production_journal_company ON production_journal_entries(company_id);")
    op.execute("CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_production_journal_product ON production_journal_entries(product_id);")
    op.execute("CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_production_stock_company ON production_stock_transactions(company_id);")
    op.execute("CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_production_stock_product ON production_stock_transactions(product_id);")
    op.execute("CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_production_stock_created ON production_stock_transactions(created_at);")
    
    # Обновляем последовательности (на случай если таблицы не пустые)
    op.execute("""
        SELECT setval('manufactured_products_id_seq', COALESCE((SELECT MAX(id) FROM manufactured_products), 1));
    """)
    op.execute("""
        SELECT setval('production_journal_entries_id_seq', COALESCE((SELECT MAX(id) FROM production_journal_entries), 1));
    """)
    op.execute("""
        SELECT setval('production_stock_transactions_id_seq', COALESCE((SELECT MAX(id) FROM production_stock_transactions), 1));
    """)
...
def downgrade() -> None:
    ...
    # Удаляем функцию (только если ее никто больше не использует)
